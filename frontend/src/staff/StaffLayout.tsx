...
import { AdminOutletContext } from "../admin/admin-context";
    ...
    return (
      <main className="mx-auto flex min-h-screen w-full max-w-xl flex-col justify-center px-6 py-10">
        <section className="space-y-4">
          ...
          <Button type="button" variant="outline" onClick={signOut}>Đăng xuất</Button>
        </section>
      </main>
    );
  }
      ...
      <header className="border-b border-border">
        <div className="mx-auto flex min-h-14 w-full max-w-7xl flex-wrap items-center justify-between gap-3 px-4 py-3">
          <div className="flex flex-wrap items-center gap-3">
            <span className="text-sm font-semibold">WebOrder Staff</span>
            <nav className="flex items-center gap-1 text-sm">
              <NavLink className={({ isActive }) => navClass(isActive)} to="/staff">
                Dashboard
              </NavLink>
              ...
              <NavLink className={({ isActive }) => navClass(isActive)} to="/staff/billing">
                Billing
              </NavLink>
